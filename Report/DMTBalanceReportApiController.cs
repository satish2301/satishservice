...
{
    public class DMTBalanceReportController : ApiController
    {
        // GET api/values
        ...
        public HttpResponseMessage GetBankDetails(string request)
        {
            ...
            try
            {
                ...
                if ((apiKey != null || apiKey != "") && requestIP != "")
                {
                    ...
                    BalanceUploadRequest objRQ = new BalanceUploadRequest();
                    BalanceUploadReponse obj = new BalanceUploadReponse();
                        if (objProcessor.IsJson)
                        {
                            objRQ = JsonConvert.DeserializeObject<BalanceUploadRequest>(requestString);
                        }
                        else
                        {
                             objRQ = XmlSerializers.DeserializeFromXml<BalanceUploadRequest>(requestString);
                        }
                    Log.Error.LogLogin(Convert.ToString(objRQ.CompanyID), Convert.ToString(requestString), "DMTPaymentReportRequest");
                    //objRQ.CompanyID = CompanyId;
                    bool validate = ValidateApiuser(objRQ.TokenId, requestIP, objRQ.CompanyID);
                    if (validate)
                    {
                        // objRQ.ApiPassword
                        obj = objProcessor.uploadBalance(objRQ);
                        if (obj != null)
                        {
                            response = JsonConvert.SerializeObject(obj);
                            string json = Convert.ToString(response);
                            Log.Error.LogLogin(Convert.ToString(objRQ.CompanyID), Convert.ToString(response), "DMTPaymentReportResponse");
