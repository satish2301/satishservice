...
namespace DMTServices.Controllers
{
    public class ValidateUserController : ApiController
    {
        ...
        public HttpResponseMessage DMTSignIns(string request)
        {
            ...
            try
            {
                ...
                if ((apiKey != null || apiKey != "") && requestIP != "")
                {
                    ...
                    ValidateUserRequest objRQ = new ValidateUserRequest();
                    ValidateUserReponse obj = new ValidateUserReponse();
                    if (objProcessor.IsJson)
                    {
                         objRQ = JsonConvert.DeserializeObject<ValidateUserRequest>(requestString);
                    }
                    else
                    {
                         objRQ = XmlSerializers.DeserializeFromXml<ValidateUserRequest>(requestString);
                    }
                    ...
                    if (validate)
                    {
                        obj = objProcessor.ValidateUserAPI(objRQ);
        ...
        {
            bool validate = false;
            if(tokenId == "Njk1MjAyMC0wMi0xMSAxMTozN")
            {
