...
{
    public class SaveDetailsController : ApiController
    {
        // GET api/values
        ...
        [AcceptVerbs("POST")]
        [HttpPost()]
        [ActionName("SaveDetails")]
        public HttpResponseMessage SaveDetails(HttpRequestMessage request)
        {
            ...
            return SaveDetails(requestString);
        }
        ...
        [AcceptVerbs("POST")]
        [HttpPost()]
        public HttpResponseMessage SaveDetails(string request)
        {
            ...
            try
            {
                ...
                if ((apiKey != null || apiKey != "") && requestIP != "")
                {
                    ...
                    SaveDetailsRequest objRQ = new SaveDetailsRequest();
                    DebitUserReponse obj = new DebitUserReponse();
                    if (objProcessor.IsJson)
                    {
                         objRQ = JsonConvert.DeserializeObject<SaveDetailsRequest>(requestString);
                    }
                    else
                    {
                         objRQ = XmlSerializers.DeserializeFromXml<SaveDetailsRequest>(requestString);
                    }
                    ...
                    if (validate)
                    {
                        obj = objProcessor.SaveDetails(objRQ);
