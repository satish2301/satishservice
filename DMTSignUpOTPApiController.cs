...
{
    public class DMTSignUpOTPApiController : ApiController
    {
        // GET api/values
        ...
        [AcceptVerbs("POST")]
        [HttpPost()]
        [ActionName("DMTSignUpOTP")]
        public HttpResponseMessage DMTSignUpOTP(HttpRequestMessage request)
        {
            ...
            Log.Error.LogLogin(Convert.ToString("Password"), Convert.ToString(Password), "DMTSignUpOTPRequest");
            try
            {
            ...
            {
                Log.Error.LogLogin(Convert.ToString("exce"), Convert.ToString(ex.Message), "DMTSignUpOTPRequest");
            }
        ...
        public HttpResponseMessage DMTSignIns(string request)
        {
            ...
            try
            {
                ...
                if ((apiKey != null || apiKey != "") && requestIP != "")
                {
                    ...
                    SignUpGETOTPRequest objRQ = new SignUpGETOTPRequest();
                    SignUpGETOTPResponse obj = new SignUpGETOTPResponse();
                    if (objProcessor.IsJson)
                    {
                         objRQ = JsonConvert.DeserializeObject<SignUpGETOTPRequest>(requestString);
                        Log.Error.LogLogin(Convert.ToString(objRQ.MobileNo), Convert.ToString(requestString), "DMTChangePINRequest");
                    }
                    else
                    {
                         objRQ = XmlSerializers.DeserializeFromXml<SignUpGETOTPRequest>(requestString);
                    }
                    CompanyId = objRQ.MobileNo;
                   ...
                   // objRQ.CompanyID = objRQ.CompanyID;
                    ...
                    if (validate)
                    {
                        obj = objProcessor.SignUpgetotp(objRQ);
                    }
                    if (obj != null && validate==true)
                    {
                        response = JsonConvert.SerializeObject(obj);
                        string json = Convert.ToString(response);
                        Log.Error.LogLogin(Convert.ToString(objRQ.MobileNo), Convert.ToString(response), "DMTSignUpOTPResponse");
