        ...
        public HttpResponseMessage GetSearchNember(string request)
        {
            ...
            try
            {
                ...
                if (request != null && (apiKey != null || apiKey != "") && requestIP != "")
                {
                    ...
                    cashdropRequest objRQ = new cashdropRequest();
                    cashdropResponse obj = new cashdropResponse();
                    if (objProcessor.IsJson)
                    {
                        objRQ = JsonConvert.DeserializeObject<cashdropRequest>(requestString);
                        Log.Error.LogLogin(Convert.ToString(CompanyId), Convert.ToString(requestString), "DMTCashPluginRequest");
                    }
                    else
                    {
                        objRQ = XmlSerializers.DeserializeFromXml<cashdropRequest>(requestString);
                    }
                    ...
                    if (validate)
                    {
                        ...
                        checkbalance = clsRechargeProcessor.GetAgentAvalable("10", objRQ.CompanyID);
                        if (checkbalance)
                            obj = objProcessor.Plugincash(objRQ);
                        else
                        {
                            obj.errorCode = "1";
                            obj.errorMsg = "Ooops!! Balance Low.";
                        }
                        if (objProcessor.IsJson)
                        {
                            response = JsonConvert.SerializeObject(obj);
                            string json = Convert.ToString(response);
                            Log.Error.LogLogin(Convert.ToString(CompanyId), Convert.ToString(json), "DMTCashPluginResponse");
