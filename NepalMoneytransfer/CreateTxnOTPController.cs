...
{
    public class CreateTxnOTPController : ApiController
    {
        // GET api/values
        ...
        public HttpResponseMessage CreateTxnOTP(HttpRequestMessage request)
        {
           ...
           // Password = Convert.ToString(request.Headers.GetValues("ApiPassword").First());
            ...
            return CreateTxnOTP(requestString);
        }
        ...
        [AcceptVerbs("POST")]
        [HttpPost()]
        public HttpResponseMessage CreateTxnOTP(string request)
        {
            ...
            try
            {
                ...
                if ((apiKey!=null || apiKey!="") && requestIP!="")
                {
                    ...
                    CreateTxnOTPRequest objRQ = new CreateTxnOTPRequest();
                    CrateReceiverOTPResponse obj = new CrateReceiverOTPResponse();
                    if (objProcessor.IsJson)
                    {
                        objRQ = JsonConvert.DeserializeObject<CreateTxnOTPRequest>(requestString);
                    }
                    else
                    {
                    ...
                    if (validate)
                    {
                        obj = objProcessor.CreateTxnOTPRequest(objRQ);
                    }
                    else
                    {
