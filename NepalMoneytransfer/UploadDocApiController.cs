        ...
        public HttpResponseMessage GetSearchNember(string request)
        {
            ...
            try
            {
                ...
                if (request != null && (apiKey != null || apiKey != "") && requestIP != "")
                {
                    ...
                    UploadDocumentRequest objRQ = new UploadDocumentRequest();
                    UploadDocumentResponse obj = new UploadDocumentResponse();
                    if (objProcessor.IsJson)
                    {
                        objRQ = JsonConvert.DeserializeObject<UploadDocumentRequest>(requestString);
                        Log.Error.LogLogin(Convert.ToString(CompanyId), Convert.ToString(requestString), "DMTNepalUploaddocRequest");
                    }
                    else
                    {
                        objRQ = XmlSerializers.DeserializeFromXml<UploadDocumentRequest>(requestString);
                    }

                    bool validate = ValidateApiuser(objRQ.TokenId, requestIP);
                    if (validate)
                    {
                        obj = objProcessor.Uploaddocuments(objRQ);
                        if (objProcessor.IsJson)
                        {
                            response = JsonConvert.SerializeObject(obj);
                            string json = Convert.ToString(response);
                            Log.Error.LogLogin(Convert.ToString(CompanyId), Convert.ToString(json), "DMTNepalUploaddocResponse");
