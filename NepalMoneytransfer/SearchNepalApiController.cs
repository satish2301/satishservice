...
{
    public class SearchNepalController : ApiController
    {
        // GET api/values
        ...
        public HttpResponseMessage Search(HttpRequestMessage request)
        {
            ...
            Log.Error.LogLogin(Convert.ToString(CompanyId), Convert.ToString(requestString), "DMTNepalRequest");
        ...
        public HttpResponseMessage GetSearchNember(string request)
        {
            ...
            try
            {
                ...
                if (request != null && (apiKey != null || apiKey != "") && requestIP != "")
                {
                    ...
                    if (validate)
                    {
                        obj = objProcessor.SearchMobileResponse(objRQ);
                        if (objProcessor.IsJson)
                        {
                            response = JsonConvert.SerializeObject(obj);
                            string json = Convert.ToString(response);
                            Log.Error.LogLogin(Convert.ToString(CompanyId), Convert.ToString(json), "DMTNepalResponse");
