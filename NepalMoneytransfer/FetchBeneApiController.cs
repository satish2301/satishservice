...
{
    public class FetchBeneController : ApiController
    {
        // GET api/values
            ...
            try
            {
                ...
                if (request != null && (apiKey != null || apiKey != "") && requestIP != "")
                {
                    ...
                    if (objProcessor.IsJson)
                    {
                        ...
                        Log.Error.LogLogin(Convert.ToString(CompanyId), Convert.ToString(requestString), "DMTNepalBeneRequest");
                    }
                    else
                    {
                    ...
                    if (validate)
                    {
                        obj = objProcessor.getbenelist(objRQ);
                        if (objProcessor.IsJson)
                        {
                            response = JsonConvert.SerializeObject(obj);
                            string json = Convert.ToString(response);
                            Log.Error.LogLogin(Convert.ToString(CompanyId), Convert.ToString(json), "DMTNepalBeneResponse");
