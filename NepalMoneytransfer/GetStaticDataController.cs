...
{
    public class GetStaticDataController : ApiController
    {
        // GET api/values
        ...
        #region GetStaticData
        [AcceptVerbs("POST")]
        [HttpPost()]
        [ActionName("TranStatus")]
        public HttpResponseMessage GetStaticData(HttpRequestMessage request)
        {
            ...
            return TranStatus(requestString);
        }
        ...
        [AcceptVerbs("POST")]
        [HttpPost()]
        public HttpResponseMessage TranStatus(string request)
        {
            ...
            try
            {
                ...
                if ((apiKey!=null || apiKey!="") && requestIP!="")
                {
                    ...
                    NepalGetdataRequest objRQ = new NepalGetdataRequest();
                    NepalGetdataResponse obj = new NepalGetdataResponse();
                    if (objProcessor.IsJson)
                    {
                        objRQ = JsonConvert.DeserializeObject<NepalGetdataRequest>(requestString);
                    }
                    else
                    {
                   ...
                   // objRQ.ApiPassword
                    obj = objProcessor.NepalGetdataGetReport(objRQ);
                    if (objProcessor.IsJson)
                    {
