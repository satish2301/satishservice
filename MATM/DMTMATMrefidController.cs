...
{
    public class DMTMATMrefidController : ApiController
    {
        // GET api/values
            ...
            try
            {
                ...
                if ((apiKey != null || apiKey != "") && requestIP != "")
                {
                    ...
                    if (objProcessor.IsJson)
                    {
                        ...
                        Log.Error.LogLogin(Convert.ToString(objRQ.CompanyID), Convert.ToString(requestString), "MATMIDRequest");
                    }
                    else
                    {
                    ...
                    {
                        // objRQ.ApiPassword
                        obj = objProcessor.MATMID(objRQ);
                        if (obj != null)
                        {
                            response = JsonConvert.SerializeObject(obj);
                            string json = Convert.ToString(response);
                            Log.Error.LogLogin(Convert.ToString(objRQ.CompanyID), Convert.ToString(json), "MATMIDResponse");
