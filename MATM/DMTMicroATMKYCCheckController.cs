...
{
    public class DMTMicroATMKYCCheckController : ApiController
    {
        // GET api/values
        ...
        public HttpResponseMessage GetTokens(string request)
        {
            ...
            try
            {
                ...
                if ((apiKey != null || apiKey != "") && requestIP != "")
                {
                    ...
                    KYCCheckRequest objRQ = new KYCCheckRequest();
                    MicroATMResponse obj = new MicroATMResponse();
                    if (objProcessor.IsJson)
                    {
                        objRQ = JsonConvert.DeserializeObject<KYCCheckRequest>(requestString);
                        Log.Error.LogLogin(Convert.ToString(objRQ.CompanyID), Convert.ToString(requestString), "MicroKYCStatusRequest");
                    }
                    else
                    {
                        objRQ = XmlSerializers.DeserializeFromXml<KYCCheckRequest>(requestString);
                    }
                    ...
                    {
                        // objRQ.ApiPassword
                        obj = objProcessor.MicroATMCheckKycStaus(objRQ);
                        if (obj != null)
                        {
                            response = JsonConvert.SerializeObject(obj);
                            string json = Convert.ToString(response);
                            Log.Error.LogLogin(Convert.ToString(objRQ.CompanyID), Convert.ToString(json), "MicroKYCStatusResponse");
