...
{
    public class DMTSendReqController : ApiController
    {
        // GET api/values
        ...
        [AcceptVerbs("POST")]
        [HttpPost()]
        [ActionName("DMTSendReq")]
        public HttpResponseMessage HandleRequestFun(HttpRequestMessage request)
        {
           ...
           // Password = Convert.ToString(request.Headers.GetValues("ApiPassword").First());
            ...
            return HnadleVerifyFun(requestString);
        }
        ...
        [AcceptVerbs("POST")]
        [HttpPost()]
        public HttpResponseMessage HnadleVerifyFun(string request)
        {
            ...
            try
            {
                ...
                if ((apiKey != null || apiKey != "") && requestIP != "")
                {
                    ...
                    PackageRequest objRQ = new PackageRequest();
                    PackageRequestResponse obj = new PackageRequestResponse();
                        if (objProcessor.IsJson)
                        {
                            objRQ = JsonConvert.DeserializeObject<PackageRequest>(requestString);
                        }
                        else
                        {
                             objRQ = XmlSerializers.DeserializeFromXml<PackageRequest>(requestString);
                        }
                    Log.Error.LogLogin(Convert.ToString(objRQ.CompanyID), Convert.ToString(requestString), "PackageRequest");
                    //objRQ.CompanyID = CompanyId;
                    bool validate = ValidateApiuser(objRQ.TokenId, requestIP, objRQ.CompanyID);
                    if (validate)
                    {
                        // objRQ.ApiPassword
                        obj = objProcessor.SendRequest(objRQ);
                       
                        if (obj != null)
                        {
                            response = JsonConvert.SerializeObject(obj);
                            string json = Convert.ToString(response);
                            Log.Error.LogLogin(Convert.ToString(objRQ.CompanyID), Convert.ToString(response), "PackageRequestResponse");
