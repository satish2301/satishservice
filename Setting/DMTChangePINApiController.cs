...
{
    public class DMTChangePINController : ApiController
    {
        // GET api/values
        ...
        #region DMTChangePassword
        [AcceptVerbs("POST")]
        [HttpPost()]
        [ActionName("DMTChangePassword")]
        public HttpResponseMessage Token(HttpRequestMessage request)
        {
            ...
            Log.Error.LogLogin(Convert.ToString("Change"), Convert.ToString(requestString), "DMTChangePINRequest");
            apiKey = Convert.ToString(request.Headers.GetValues("apikey").First());
            Log.Error.LogLogin(Convert.ToString(apiKey), Convert.ToString(requestString), "DMTChangePINRequest");
            ...
            Log.Error.LogLogin(Convert.ToString("Password"), Convert.ToString(Password), "DMTChangePINRequest");
            try
            {
            ...
            {
                Log.Error.LogLogin(Convert.ToString("exce"), Convert.ToString(ex.Message), "DMTChangePINRequest");
            }
            Log.Error.LogLogin(Convert.ToString("finalcheck"), Convert.ToString(Password+ " Channel:" + Channel+ "deviceID:" + deviceID+ ":apiKey"+ apiKey), "DMTChangePINRequest");
        ...
        public HttpResponseMessage DMTSignIns(string request)
        {
            ...
            try
            {
                ...
                if ((apiKey != null || apiKey != "") && requestIP != "")
                {
                    ...
                    ChangepinRequest objRQ = new ChangepinRequest();
                    ChangepinResponse obj = new ChangepinResponse();
                    if (objProcessor.IsJson)
                    {
                         objRQ = JsonConvert.DeserializeObject<ChangepinRequest>(requestString);
                        Log.Error.LogLogin(Convert.ToString(objRQ.CompanyID), Convert.ToString(requestString), "DMTChangePINRequest");
                    }
                    else
                    {
                         objRQ = XmlSerializers.DeserializeFromXml<ChangepinRequest>(requestString);
                    }
                    ...
                    objRQ.OldPIN = objRQ.OldPIN;
                    objRQ.NewPIN= Globals.Encrypt(objRQ.NewPIN).Trim();

                    // objRQ.ApiPassword
                    if (validate)
                    {
                        obj = objProcessor.ChangePin(objRQ);
                    }
