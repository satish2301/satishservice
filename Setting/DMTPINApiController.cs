...
{
    public class DMTPINController : ApiController
    {
        // GET api/values
        ...
        public HttpResponseMessage GetTokens(string request)
        {
            ...
            try
            {
                ...
                if ((apiKey != null || apiKey != "") && requestIP != "")
                {
                        ...
                        PINRequest objRQ = new PINRequest();
                        PINResponse obj = new PINResponse();
                        if (objProcessor.IsJson)
                        {
                            objRQ = JsonConvert.DeserializeObject<PINRequest>(requestString);
                        }
                        else
                        {
                             objRQ = XmlSerializers.DeserializeFromXml<PINRequest>(requestString);
                        }
                    Log.Error.LogLogin(Convert.ToString(objRQ.CompanyID), Convert.ToString(requestString), "DMTTPINVerifyRequest");
                    ...
                    {
                        // objRQ.ApiPassword
                        obj = objProcessor.ValidatePIN(objRQ);
                        if (obj != null)
                        {
                            response = JsonConvert.SerializeObject(obj);
                            string json = Convert.ToString(response);
                            Log.Error.LogLogin(Convert.ToString(objRQ.CompanyID), Convert.ToString(json), "DMTTPINVerifyResponse");
