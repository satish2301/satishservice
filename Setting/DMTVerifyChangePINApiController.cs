...
{
    public class DMTVerifyChangePINController : ApiController
    {
        // GET api/values
        ...
        public HttpResponseMessage Token(HttpRequestMessage request)
        {
            ...
            Log.Error.LogLogin(Convert.ToString("finalcheck"), Convert.ToString(requestString), "DMTChangePINVerifyRequest");
            try
            {
                apiKey = Convert.ToString(request.Headers.GetValues("apikey").First());
                Log.Error.LogLogin(Convert.ToString("finalcheckapikey"), Convert.ToString(apiKey), "DMTChangePINVerifyRequest");
          ...
          //  Password = Convert.ToString(request.Headers.GetValues("ApiPassword").First());
            ...
            {
                Log.Error.LogLogin(Convert.ToString("finalcheck"), Convert.ToString(ex.Message), "DMTChangePINVerifyRequest");
            }
            Log.Error.LogLogin(Convert.ToString("finalcheck"), Convert.ToString(CompanyId + " Channel:" + Channel + "deviceID:" + deviceID + ":apiKey" + apiKey), "DMTChangePINVerifyRequest");
        ...
        public HttpResponseMessage DMTSignIns(string request)
        {
            ...
            try
            {
                ...
                if ((apiKey != null || apiKey != "") && requestIP != "")
                {
                    ...
                    objRQ.OldPIN = Password;
                    objRQ.NewPIN = Globals.Encrypt(objRQ.NewPIN).Trim();
                    // objRQ.ApiPassword
                    if (validate)
                    {
                        obj = objProcessor.VerifyChangePinOTP(objRQ);
                    }
