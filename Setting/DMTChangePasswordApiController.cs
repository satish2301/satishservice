...
{
    public class DMTChangePasswordController : ApiController
    {
        // GET api/values
        ...
        public HttpResponseMessage Token(HttpRequestMessage request)
        {
            ...
            Log.Error.LogLogin(Convert.ToString("finalcheck"), Convert.ToString(requestString), "DMTChangepassRequest");
        ...
        public HttpResponseMessage DMTSignIns(string request)
        {
            ...
            try
            {
                ...
                if ((apiKey != null || apiKey != "") && requestIP != "")
                {
                    ...
                    ChangepasswordRequest objRQ = new ChangepasswordRequest();
                    ChangepasswordResponse obj = new ChangepasswordResponse();
                    if (objProcessor.IsJson)
                    {
                         objRQ = JsonConvert.DeserializeObject<ChangepasswordRequest>(requestString);
                    }
                    else
                    {
                         objRQ = XmlSerializers.DeserializeFromXml<ChangepasswordRequest>(requestString);
                    }
                    ...
                    Password = Globals.Encrypt(objRQ.OldPassword).Trim();
                    objRQ.OldPassword = Password;
                    objRQ.NewPassword= Globals.Encrypt(objRQ.NewPassword).Trim();

                    // objRQ.ApiPassword
                    if (validate)
                    {
                        obj = objProcessor.ChangePassword(objRQ);
                    }
