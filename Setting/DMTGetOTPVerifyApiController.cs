...
{
    public class DMTGetOTPVerifyController : ApiController
    {
        // GET api/values
        ...
        public HttpResponseMessage Token(HttpRequestMessage request)
        {
            ...
            Log.Error.LogLogin(Convert.ToString("Password"), Convert.ToString(Password), "DMTGETOTPVerifyRequest");
            try
            {
            }
            catch (Exception ex)
            {
                Log.Error.LogLogin(Convert.ToString("exce"), Convert.ToString(ex.Message), "DMTGETOTPVerifyRequest");
            }
