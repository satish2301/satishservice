...
{
    public class DMTtransactionController : ApiController
    {
        // GET api/values
        ...
        public HttpResponseMessage Token(HttpRequestMessage request)
        {
            ...
            Log.Error.LogLogin(Convert.ToString("finalcheck"), Convert.ToString(requestString), "DMTTransactionSearchRequest");
        ...
        public HttpResponseMessage GetTokens(string request)
        {
            ...
            try
            {
                ...
                if ((apiKey != null || apiKey != "") && requestIP != "")
                {
                        ...
                        ReportRequest objRQ = new ReportRequest();
                        TransReport obj = new TransReport();
                        if (objProcessor.IsJson)
                        {
                            objRQ = JsonConvert.DeserializeObject<ReportRequest>(requestString);
                        Log.Error.LogLogin(Convert.ToString(objRQ.CompanyID), Convert.ToString(requestString), "DMTTransReportRequest");

                    }
                    else
                        {
                             objRQ = XmlSerializers.DeserializeFromXml<ReportRequest>(requestString);
                        }
                    ...
                    {
                        // objRQ.ApiPassword
                        obj = objProcessor.GetReport(objRQ);
                        if (obj != null)
                        {
                            response = JsonConvert.SerializeObject(obj);
                            string json = Convert.ToString(response);
                            Log.Error.LogLogin(Convert.ToString(objRQ.CompanyID), Convert.ToString(requestString), "DMTTransReportResponse");
