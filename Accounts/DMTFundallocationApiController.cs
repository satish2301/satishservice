...
{
    public class DMTFundallocationController : ApiController
    {
        // GET api/values
        ...
        public HttpResponseMessage GetTokens(string request)
        {
            ...
            try
            {
                ...
                if ((apiKey != null || apiKey != "") && requestIP != "")
                {
                    ...
                    FundallocationRequest objRQ = new FundallocationRequest();
                    FundallocationResponse obj = new FundallocationResponse();
                        if (objProcessor.IsJson)
                        {
                            objRQ = JsonConvert.DeserializeObject<FundallocationRequest>(requestString);
                        }
                        else
                        {
                             objRQ = XmlSerializers.DeserializeFromXml<FundallocationRequest>(requestString);
                        }
                    Log.Error.LogLogin(Convert.ToString(objRQ.CompanyID), Convert.ToString(requestString), "FundallocationRequest");
                    ...
                    if (validate)
                    {
                        ...
                        checkbalance = clsRechargeProcessor.GetAgentAvalable(objRQ.UpdateLimit.ToString(), objRQ.CompanyID);
                        if(checkbalance)
                        obj = objProcessor.Fundallocation(objRQ);
                        else
                        {
                            obj.errorCode = "2";
                        ...
                        if (obj != null)
                        {
                            ...
                            Log.Error.LogLogin(Convert.ToString(objRQ.CompanyID), Convert.ToString(response), "FundallocationResponse");
