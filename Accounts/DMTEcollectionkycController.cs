...
{
    public class DMTEcollectionkycController : ApiController
    {
        // GET api/values
        ...
        public HttpResponseMessage GetTokens(string request)
        {
            ...
            try
            {
                ...
                if ((apiKey != null || apiKey != "") && requestIP != "")
                {
                    ...
                    ecollectionrequest objRQ = new ecollectionrequest();
                    EcollectionResponse obj = new EcollectionResponse();
                        if (objProcessor.IsJson)
                        {
                            objRQ = JsonConvert.DeserializeObject<ecollectionrequest>(requestString);
                        Log.Error.LogLogin(Convert.ToString(objRQ.CompanyID), Convert.ToString(requestString), "DMTEcollectionRequest");

                    }
                    else
                        {
                             objRQ = XmlSerializers.DeserializeFromXml<ecollectionrequest>(requestString);
                        }
                    ...
                    {
                        // objRQ.ApiPassword
                       obj = objProcessor.Ecollection(objRQ);
                        if (obj != null)
                        {
                            response = JsonConvert.SerializeObject(obj);
                            string json = Convert.ToString(response);
                            Log.Error.LogLogin(Convert.ToString(objRQ.CompanyID), Convert.ToString(json), "DMTEcollectionResponse");
