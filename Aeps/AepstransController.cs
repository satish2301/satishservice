...
{
    public class AepstransController : ApiController
    {
        // GET api/values
        ...
        [AcceptVerbs("POST")]
        [HttpPost()]
        [ActionName("Aepstrans")]
        public HttpResponseMessage Aepstrans(HttpRequestMessage request)
        {
           ...
           // Password = Convert.ToString(request.Headers.GetValues("ApiPassword").First());
            ...
            return Aepstrans(requestString);
        }
        ...
        [AcceptVerbs("POST")]
        [HttpPost()]
        public HttpResponseMessage Aepstrans(string request)
        {
            ...
            try
            {
                ...
                if ((apiKey!=null || apiKey!="") && requestIP!="")
                {
                    ...
                    AepsTransRequest objRQ = new AepsTransRequest();
                    AepsTransResponse obj = new AepsTransResponse();
                    if (objProcessor.IsJson)
                    {
                        objRQ = JsonConvert.DeserializeObject<AepsTransRequest>(requestString);
                    }
                    else
                    {
                    ...
                    Log.Error.LogLogin(Convert.ToString(CompanyId), Convert.ToString(requestString), "DMTAEPSTrans");
                    bool validate = ValidateApiuser(objRQ.TokenID, requestIP);
                    if (validate)
                    {
                        obj = objProcessor.AEPSTrans(objRQ);
                    }
                    else
                    {
