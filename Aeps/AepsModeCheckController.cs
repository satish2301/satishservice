...
{
    public class AepsModeCheckController : ApiController
    {
        // GET api/values
        ...
        public HttpResponseMessage AepsSearch(string request)
        {
            ...
            try
            {
                ...
                if ((apiKey!=null || apiKey!="") && requestIP!="")
                {
                    ...
                    Log.Error.LogLogin(Convert.ToString(CompanyId), Convert.ToString("AepsModeCheck" + "URL : " + requestString), "KYCAEPSModeStatusrequest");
                    bool validate = ValidateApiuser(objRQ.TokenID, requestIP); 
                    if (validate)
                    {
                        obj = objProcessor.AEPSModeCheck(objRQ);
                    }
                    else
                    {
