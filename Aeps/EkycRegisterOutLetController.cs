...
{
    public class EkycRegisterOutLetController : ApiController
    {
        // GET api/values
        ...
        #region EkycRegisterOutLet
        [AcceptVerbs("POST")]
        [HttpPost()]
        [ActionName("EkycRegisterOutLet")]
        ...
        public HttpResponseMessage AepsSearch(string request)
        {
            ...
            try
            {
                ...
                if ((apiKey!=null || apiKey!="") && requestIP!="")
                {
                    ...
                    EkycRegisterOutLetRequest objRQ = new EkycRegisterOutLetRequest();
                    EkycRegisterOutLetReponse obj = new EkycRegisterOutLetReponse();
                    if (objProcessor.IsJson)
                    {
                        objRQ = JsonConvert.DeserializeObject<EkycRegisterOutLetRequest>(requestString);
                    }
                    else
                    {
                    ...
                    Log.Error.LogLogin(Convert.ToString(CompanyId), Convert.ToString("EkycRegisterOutLet" + "URL : " + requestString), "EkycRegisterOutLetrequest");
                    bool validate = ValidateApiuser(objRQ.TokenID, requestIP); 
                    if (validate)
                    {
                        obj = objProcessor.EkycRegisterOutLet(objRQ);
                    }
                    else
                    {
                    ...
                    if (objProcessor.IsJson)
                    {
                        ...
                        Log.Error.LogLogin(Convert.ToString(objRQ.CompanyID), Convert.ToString(json), "EkycRegisterOutLetResponse");
