...
{
    public class AepBankListController : ApiController
    {
        // GET api/values
        ...
        [AcceptVerbs("POST")]
        [HttpPost()]
        [ActionName("AepBankList")]
        public HttpResponseMessage AepBankList(HttpRequestMessage request)
        {
           ...
           // Password = Convert.ToString(request.Headers.GetValues("ApiPassword").First());
            ...
            return AepBankList(requestString);
        }
        ...
        [AcceptVerbs("POST")]
        [HttpPost()]
        public HttpResponseMessage AepBankList(string request)
        {
            ...
            try
            {
                ...
                if ((apiKey!=null || apiKey!="") && requestIP!="")
                {
                    ...
                    AepBankListRequest objRQ = new AepBankListRequest();
                    AepBankListResponse obj = new AepBankListResponse();
                    if (objProcessor.IsJson)
                    {
                        objRQ = JsonConvert.DeserializeObject<AepBankListRequest>(requestString);
                    }
                    else
                    {
                    ...
                    Log.Error.LogLogin(Convert.ToString(CompanyId), Convert.ToString(requestString), "DMTAepBankList");
                    bool validate = ValidateApiuser(objRQ.TokenID, requestIP);
                    if (validate)
                    {
                        obj = objProcessor.AepBankList(objRQ);
                    }
                    else
                    {
                    ...
                    if (objProcessor.IsJson)
                    {
                        ...
                        Log.Error.LogLogin(Convert.ToString(CompanyId), Convert.ToString(response), "DMTAepBankListresp");
