...
{
    public class registertmidauthController : ApiController
    {
        // GET api/values
        ...
        [AcceptVerbs("POST")]
        [HttpPost()]
        [ActionName("registertmidauth")]
        public HttpResponseMessage registertmidauth(HttpRequestMessage request)
        {
           ...
           // Password = Convert.ToString(request.Headers.GetValues("ApiPassword").First());
            ...
            return registertmidauth(requestString);
        }
        ...
        [AcceptVerbs("POST")]
        [HttpPost()]
        public HttpResponseMessage registertmidauth(string request)
        {
            ...
            try
            {
                ...
                {
                    Log.Error.LogLogin(Convert.ToString(CompanyId), Convert.ToString(requestString), "DMTAEPSRegister");
                    ...
                    if (validate)
                    {
                        if (objRQ.APIMode == "AEPS2")
                            ...
                            obj = objProcessor.RegisterMID(objRQ);
                    ...
                    if (objProcessor.IsJson)
                    {
                        ...
                        Log.Error.LogLogin(Convert.ToString(objRQ.CompanyID), Convert.ToString(json), "DMTAEPSRegisterResponse");
