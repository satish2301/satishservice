...
{
    public class DMTAgentDetailsController : ApiController
    {
        // GET api/values
        ...
        public HttpResponseMessage GetTokens(string request)
        {
            ...
            try
            {
                ...
                if ((apiKey != null || apiKey != "") && requestIP != "")
                {
                    ...
                    AgentdetailsRequest objRQ = new AgentdetailsRequest();
                    AgentdetailsResponse obj = new AgentdetailsResponse();
                        if (objProcessor.IsJson)
                        {
                            objRQ = JsonConvert.DeserializeObject<AgentdetailsRequest>(requestString);
                        }
                        else
                        {
                             objRQ = XmlSerializers.DeserializeFromXml<AgentdetailsRequest>(requestString);
                        }
                    ...
                    {
                        // objRQ.ApiPassword
                        obj = objProcessor.GetAgentdetails(objRQ);
                        if (obj != null)
                        {
