...
{
    public class DMTDashboardController : ApiController
    {
        // GET api/values
        ...
        public HttpResponseMessage GetTokens(string request)
        {
            ...
            try
            {
                ...
                if ((apiKey != null || apiKey != "") && requestIP != "")
                {
                        ...
                        LedgerRequest objRQ = new LedgerRequest();
                    DashboardReport obj = new DashboardReport();
                        if (objProcessor.IsJson)
                        {
                            objRQ = JsonConvert.DeserializeObject<LedgerRequest>(requestString);
                        Log.Error.LogLogin(Convert.ToString(objRQ.CompanyID), Convert.ToString(requestString), "DMTDashboardRequest");
                    }
                        else
                        {
                             objRQ = XmlSerializers.DeserializeFromXml<LedgerRequest>(requestString);
                        }
                    ...
                    {
                        // objRQ.ApiPassword
                        obj = objProcessor.GetDashboradReport(objRQ);
                        if (obj != null)
                        {
