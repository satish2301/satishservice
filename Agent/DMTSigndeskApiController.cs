...
{
    public class DMTSigndeskApiController : ApiController
    {
        // GET api/values
        ...
        public HttpResponseMessage GetTokens(string request)
        {
            ...
            try
            {
                ...
                if ((apiKey != null || apiKey != "") && requestIP != "")
                {
                    ...
                    NewregistrationRquest objRQ = new NewregistrationRquest();
                    NewregistrationRquest obj = new NewregistrationRquest();
                        if (objProcessor.IsJson)
                        {
                            objRQ = JsonConvert.DeserializeObject<NewregistrationRquest>(requestString);
                        Log.Error.LogLogin(Convert.ToString(objRQ.CompanyID), Convert.ToString(requestString), "DMTSigndeskApiRequest");

                    }
                    else
                        {
                             objRQ = XmlSerializers.DeserializeFromXml<NewregistrationRquest>(requestString);
                        }
                    ...
                    {
                        // objRQ.ApiPassword
                       obj = objProcessor.SignDeskVerification(objRQ);
                        if (obj != null)
                        {
                            response = JsonConvert.SerializeObject(obj);
                            string json = Convert.ToString(response);
                            Log.Error.LogLogin(Convert.ToString(objRQ.CompanyID), Convert.ToString(json), "DMTSigndeskApiResponse");
                        ...
                        {
                            return new HttpResponseMessage { Content = new StringContent("{\"Response\": { \"Status\": \"Fail\",\"Message\": \"Error in verification\" }}", System.Text.Encoding.UTF8, "application/json") };
