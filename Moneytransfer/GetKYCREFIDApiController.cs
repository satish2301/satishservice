...
{
    public class GetKYCREFIDController : ApiController
    {
        // GET api/values
        ...
        public HttpResponseMessage GetRefundOTP(string request)
        {
            ...
            try
            {
                ...
                if (request != null)
                {
                    ...
                    KYCURLRequest objRQ = new KYCURLRequest();
                    KYCURLResponse obj = new KYCURLResponse();
                    if (objProcessor.IsJson)
                    {
                        objRQ = JsonConvert.DeserializeObject<KYCURLRequest>(requestString);
                    }
                    else
                    {
                        objRQ = XmlSerializers.DeserializeFromXml<KYCURLRequest>(requestString);
                    }
                    Log.Error.LogLogin(Convert.ToString(objRQ.agentCode), Convert.ToString(requestString), "DMTKYCURLRequestRequest");

                    bool validate = ValidateApiuser(objRQ.TokenId, requestIP);
                    if (validate)
                    {
                        obj = objProcessor.GetKYCREFID(objRQ);
                        if (objProcessor.IsJson)
                        {
                            response = JsonConvert.SerializeObject(obj);
                            string json = Convert.ToString(response);
                            Log.Error.LogLogin(Convert.ToString(objRQ.mobileNo), Convert.ToString(json), "DMTKYCURLRequestResponse");
