            ...
            try
            {
                ...
                if (request != null)
                {
                    ...
                    if (objProcessor.IsJson)
                    {
                        ...
                        Log.Error.LogLogin(Convert.ToString(objRQ.CompanyID), Convert.ToString(requestString), "DMTVerifyRequest");
                    }
                    else
                    {
                    ...
                    if (validate)
                    {
                        if (Channel.ToLower() == "app" && Usertype != "1")
                            obj = objProcessor.VerifyAccountAegnt(objRQ);
                        else if(Usertype=="1")
                            obj = objProcessor.VerifyAccountAegnt(objRQ);

                        else
                            obj = objProcessor.VerifyAccount(objRQ);
                        ...
                        if (objProcessor.IsJson)
                        {
                            ...
                            Log.Error.LogLogin(Convert.ToString(objRQ.CompanyID), Convert.ToString(response), "DMTVerifyResponse");
