    ...
    public class CreditCardController : ApiController
    {
        ...
        public HttpResponseMessage Transfer(HttpRequestMessage request)
        {
            ...
            try
            {
                ...
                Log.Error.LogLogin(Convert.ToString("Usertype"), Convert.ToString(" Usertype:" + Usertype), "DMTCCRequest");
            }
            catch (Exception ex)
            {
            }
            Log.Error.LogLogin(Convert.ToString("FinalCCrequest"), Convert.ToString(requestString+" Usertypr:"+ Usertype), "DMTCCRequest");
            ...
            try
            {
                ...
                if (request != null)
                {
                    ...
                    if (objProcessor.IsJson)
                    {
                        ...
                        Log.Error.LogLogin(Convert.ToString(objRQ.CompanyID), Convert.ToString(requestString), "DMTCCTransRequest");
                    }
                    else
                    {
                    ...
                    if (validate)
                    {
                        ...
                        {
                            if (Convert.ToDecimal(objRQ.TransferAmount) < 49801 && Convert.ToDecimal(objRQ.TransferAmount) > 99)
                            {
                                if (Channel.ToLower() == "app" && Usertype != "1")
                                    obj = objProcessor.TransferCreditCardAgent(objRQ);
                                else if (Usertype == "1")
                                {
                            ...
                            {
                                obj.errorCode = "2";
                                obj.errorMsg = "Ooops!! The Amount cannot be transferred more than 49800 thousand! or less then 100";
                            }
                        }
                        else
                        {
