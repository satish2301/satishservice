...
{
    public class FetchAllController : ApiController
    {
        // GET api/values
        ...
        public HttpResponseMessage GetSearchNember(string request)
        {
            ...
            try
            {
                ...
                if (request != null && (apiKey != null || apiKey != "") && requestIP != "")
                {
                    ...
                    Log.Error.LogLogin(Convert.ToString(CompanyId), Convert.ToString(requestString), "DMTbenefetchRequest");

                    bool validate = ValidateApiuser(objRQ.TokenId, requestIP);
                    if (validate)
                    {
                        obj = objProcessor.FetchAllMobileResponse(objRQ);
                        if (objProcessor.IsJson)
                        {
                            response = JsonConvert.SerializeObject(obj);
                            string json = Convert.ToString(response);
                            Log.Error.LogLogin(Convert.ToString(CompanyId), Convert.ToString(json), "DMTbenefetchResponse");
