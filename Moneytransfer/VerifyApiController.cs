...
namespace DMTServices.Controllers
{
    public class VerifyController : ApiController
    {
        ...
        public HttpResponseMessage VerifyOTP(HttpRequestMessage request)
        {
            ...
            return GetVerifyOTP(requestString);
        }

        // public HttpResponseMessage GetHotels([FromBody]Hotel_RQ request)
        [AcceptVerbs("POST")]
        [HttpPost()]
        public HttpResponseMessage GetVerifyOTP(string request)
        {
            ...
            try
            {
                ...
                if (request != null)
                {
                    ...
                    if (objProcessor.IsJson)
                    {
                        ...
                        Log.Error.LogLogin(Convert.ToString(objRQ.CompanyID), Convert.ToString(requestString), "DMTVerifyOTPRequest");
                    }
                    else
                    {
                    ...
                    if (validate)
                    {
                        obj = objProcessor.VerifyOTP(objRQ);
                        if (objProcessor.IsJson)
                        {
                            response = JsonConvert.SerializeObject(obj);
                            string json = Convert.ToString(response);
                            Log.Error.LogLogin(Convert.ToString(objRQ.CompanyID), Convert.ToString(response), "DMTVerifyOTPResponse");
