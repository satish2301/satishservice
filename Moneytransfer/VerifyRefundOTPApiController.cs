...
{
    public class VerifyRefundOTPController : ApiController
    {
        // GET api/values
        ...
        #region Veriffy Remitter OTP
        [AcceptVerbs("POST")]
        [HttpPost()]
        [ActionName("VerifyRefundOTP")]
        public HttpResponseMessage VerifyRefundOTP(HttpRequestMessage request)
        {
            ...
            return VerifyRefundOTP(requestString);
        }

        // public HttpResponseMessage GetHotels([FromBody]Hotel_RQ request)
        [AcceptVerbs("POST")]
        [HttpPost()]
        public HttpResponseMessage VerifyRefundOTP(string request)
        {
            ...
            try
            {
                ...
                if (request != null)
                {
                    ...
                    if (objProcessor.IsJson)
                    {
                        ...
                        Log.Error.LogLogin(Convert.ToString(objRQ.CompanyID), Convert.ToString(requestString), "dmtotprefundRequest");
                    }
                    else
                    {
                    ...
                    if (validate)
                    {
                        obj = objProcessor.RefundgetotpVerify(objRQ);
                        Log.Error.LogLogin(Convert.ToString(objRQ.CompanyID), Convert.ToString(obj), "dmtotprefundResponse");
                        if (objProcessor.IsJson)
                        {
