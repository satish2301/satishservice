...
{
    public class DeletebeneController : ApiController
    {
        // GET api/values
...
# region Deletebene
        ...
        [HttpPost()]
        [ActionName("Transfer")]
        public HttpResponseMessage Deletebene(HttpRequestMessage request)
        {
            ...
            try
            {
                ...
                Log.Error.LogLogin(Convert.ToString("Usertype"), Convert.ToString(" Usertype:" + Usertype), "DMTDeletebeneRequest");
            }
            catch (Exception ex)
            {
            }
            Log.Error.LogLogin(Convert.ToString("Finalrequest"), Convert.ToString(requestString+" Usertypr:"+ Usertype), "DMTDeletebeneRequest");
            ...
            return Deletebene(requestString);
        }

      
        [AcceptVerbs("POST")]
        [HttpPost()]
        public HttpResponseMessage Deletebene(string request)
        {
            ...
            try
            {
                ...
                if (request != null)
                {
                    ...
                    if (objProcessor.IsJson)
                    {
                        ...
                        Log.Error.LogLogin(Convert.ToString(objRQ.CompanyID), Convert.ToString(requestString), "DMTDeletebeneRequest");
                    }
                    else
                    {
                    ...
                    if (validate)
                    {
                        obj = objProcessor.DeleteBene(objRQ);
                        if (objProcessor.IsJson)
                        {
                            response = JsonConvert.SerializeObject(obj);
                            string json = Convert.ToString(response);
                            Log.Error.LogLogin(Convert.ToString(objRQ.CompanyID), Convert.ToString(json), "DMTDeleteBeneResponse");
