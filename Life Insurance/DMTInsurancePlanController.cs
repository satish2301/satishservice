...
{
    public class DMTInsurancePlanController : ApiController
    {
        // GET api/values
        ...
        [AcceptVerbs("POST")]
        [HttpPost()]
        [ActionName("DMTInsurancePlan")]
        public HttpResponseMessage HandleQueryData(HttpRequestMessage request)
        {
            ...
            return HandleResponse(requestString);
        }
        ...
        [AcceptVerbs("POST")]
        [HttpPost()]
        public HttpResponseMessage HandleResponse(string request)
        {
            ...
            try
            {
                ...
                if ((apiKey != null || apiKey != "") && requestIP != "")
                {
                    ...
                    LifeInsuRequest objRQ = new LifeInsuRequest();
                    LifeInsuPlanResponse obj = new LifeInsuPlanResponse();
                    if (objProcessor.IsJson)
                    {
                        objRQ = JsonConvert.DeserializeObject<LifeInsuRequest>(requestString);
                    }
                    else
                    {
                        objRQ = XmlSerializers.DeserializeFromXml<LifeInsuRequest>(requestString);
                    }
                    Log.Error.LogLogin(Convert.ToString(objRQ.CompanyID), Convert.ToString(requestString), "LifeInsuRequest");
                    //objRQ.CompanyID = CompanyId;
                    bool validate = ValidateApiuser(objRQ.TokenId, requestIP, objRQ.CompanyID);
                    if (validate)
                    {
                        // objRQ.ApiPassword
                        obj = objProcessor.GetInsuPlan(objRQ);

                        if (obj != null)
                        {
                            response = JsonConvert.SerializeObject(obj);
                            string json = Convert.ToString(response);
                            Log.Error.LogLogin(Convert.ToString(objRQ.CompanyID), Convert.ToString(response), "LifeInsuPlanResponse");
