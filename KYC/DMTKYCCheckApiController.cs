...
{
    public class DMTKYCCheckController : ApiController
    {
        // GET api/values
        ...
        public HttpResponseMessage GetTokens(string request)
        {
            ...
            try
            {
                ...
                if ((apiKey != null || apiKey != "") && requestIP != "")
                {
                        ...
                        Log.Error.LogLogin(Convert.ToString(objRQ.CompanyID), Convert.ToString(requestString), "NSDLKYCStatusRequest");
                    }
                        else
                        {
                    ...
                    {
                        // objRQ.ApiPassword
                        obj = objProcessor.CheckKycStaus(objRQ);
                        if (obj != null)
                        {
                            response = JsonConvert.SerializeObject(obj);
                            string json = Convert.ToString(response);
                            Log.Error.LogLogin(Convert.ToString(objRQ.CompanyID), Convert.ToString(json), "NSDLKYCStatusResponse");
