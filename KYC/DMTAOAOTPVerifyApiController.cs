...
{
    public class DMTAOAOTPVerifyController : ApiController
    {
        // GET api/values
        ...
        #region DMTAOAOTPVerify
        [AcceptVerbs("POST")]
        [HttpPost()]
        [ActionName("DMTAOAOTPVerify")]
        public HttpResponseMessage Token(HttpRequestMessage request)
        {
            ...
            Log.Error.LogLogin(Convert.ToString("Password"), Convert.ToString(Password), "DMTAOAOTPVerifyRequest");
            try
            {
            }
            catch (Exception ex)
            {
                Log.Error.LogLogin(Convert.ToString("exce"), Convert.ToString(ex.Message), "DMTAOAOTPVerifyRequest");
            }
            Log.Error.LogLogin(Convert.ToString("finalcheck"), Convert.ToString(Password+ " Channel:" + Channel+ "deviceID:" + deviceID+ ":apiKey"+ apiKey), "DMTGETOTPVerifyRequest");
            ...
            try
            {
                ...
                if ((apiKey != null || apiKey != "") && requestIP != "")
                {
                    ...
                    if (objProcessor.IsJson)
                    {
                        ...
                        Log.Error.LogLogin(Convert.ToString(objRQ.CompanyID), Convert.ToString(requestString), "DMTAOAOTPVerifyRequest");
                    }
                    else
                    {
                    ...
                    if (validate)
                    {
                        ...
                        obj = objProcessor.sendvalidateotp(objRQ);
                        else
                            obj = objProcessor.Veryotpaadhar(objRQ);
                    ...
                    if (obj != null && validate==true)
                    {
                        ...
                        Log.Error.LogLogin(Convert.ToString(objRQ.CompanyID), Convert.ToString(response), "DMTAOAOTPVerifyResponse");
