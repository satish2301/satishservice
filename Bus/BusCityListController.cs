...
{
    public class BusCityListController : ApiController
    {
        // GET api/values
        ...
        [AcceptVerbs("POST")]
        [HttpPost()]
        [ActionName("BusCityList")]
        public HttpResponseMessage BusCityList(HttpRequestMessage request)
        {
           ...
           // Password = Convert.ToString(request.Headers.GetValues("ApiPassword").First());
            ...
            return BusCityList(requestString);
        }
        ...
        [AcceptVerbs("POST")]
        [HttpPost()]
        public HttpResponseMessage BusCityList(string request)
        {
            ...
            try
            {
                ...
                if ((apiKey!=null || apiKey!="") && requestIP!="")
                {
                    ...
                    clsBusRequestProcessor objProcessor = new clsBusRequestProcessor();
                   
                    if (string.Compare(requestContentType, "application/json") == 0)
                        objProcessor.IsJson = true;


                    BusCityRequest objRQ = new BusCityRequest();
                    BusCityListReponse obj = new BusCityListReponse();
                    if (objProcessor.IsJson)
                    {
                        objRQ = JsonConvert.DeserializeObject<BusCityRequest>(requestString);
                    }
                    else
                    {
                    ...
                    if (validate)
                    {
                        obj = objProcessor.BusCityList(objRQ);
                    }
                    else
                    {
                        response = JsonConvert.SerializeObject(obj);
                        string json = Convert.ToString(response);
                        return new HttpResponseMessage { Content = new StringContent("{\"Response\": { \"Status\": \"Fail\",\"Message\": \"Invalid Toaken\" },\"Data\":" + json + "}", System.Text.Encoding.UTF8, "application/json") };
