...
namespace DMTServices.Controllers
{
    public class SeatLayoutController : ApiController
    {
        ...
        [AcceptVerbs("POST")]
        [HttpPost()]
        [ActionName("SeatLayout")]
        public HttpResponseMessage SeatLayout(HttpRequestMessage request)
        {
           ...
           // Password = Convert.ToString(request.Headers.GetValues("ApiPassword").First());
            ...
            return SeatLayout(requestString);
        }
        ...
        [AcceptVerbs("POST")]
        [HttpPost()]
        public HttpResponseMessage SeatLayout(string request)
        {
            ...
            try
            {
                ...
                if ((apiKey!=null || apiKey!="") && requestIP!="")
                {
                    ...
                    SeatLayOutRequest objRQ = new SeatLayOutRequest();
                    CommonSeatLayoutResponse obj = new CommonSeatLayoutResponse();
                    if (objProcessor.IsJson)
                    {
                        objRQ = JsonConvert.DeserializeObject<SeatLayOutRequest>(requestString);
                    }
                    else
                    {
                    ...
                    if (validate)
                    {
                        obj = objProcessor.SeatLayout(objRQ);
                    }
                    else
                    {
