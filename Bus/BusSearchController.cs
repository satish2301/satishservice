...
{
    public class BusSearchController : ApiController
    {
        // GET api/values
        ...
        [AcceptVerbs("POST")]
        [HttpPost()]
        [ActionName("BusSearch")]
        public HttpResponseMessage BusSearch(HttpRequestMessage request)
        {
           ...
           // Password = Convert.ToString(request.Headers.GetValues("ApiPassword").First());
            ...
            return BusSearch(requestString);
        }
        ...
        [AcceptVerbs("POST")]
        [HttpPost()]
        public HttpResponseMessage BusSearch(string request)
        {
            ...
            try
            {
                ...
                if ((apiKey!=null || apiKey!="") && requestIP!="")
                {
                    ...
                    BusSearchRequest objRQ = new BusSearchRequest();
                    CommonBusSearchResponse obj = new CommonBusSearchResponse();
                    if (objProcessor.IsJson)
                    {
                        objRQ = JsonConvert.DeserializeObject<BusSearchRequest>(requestString);
                    }
                    else
                    {
                    ...
                    if (validate)
                    {
                        obj = objProcessor.BusSearch(objRQ);
                    }
                    else
                    {
