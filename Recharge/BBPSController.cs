...
{
    public class BBPSController : ApiController
    {
        // GET api/values
        ...
        public HttpResponseMessage StateList(HttpRequestMessage request)
        {
           ...
           // Password = Convert.ToString(request.Headers.GetValues("ApiPassword").First());
            ...
            return StateList(requestString);
        }
        ...
        [AcceptVerbs("POST")]
        [HttpPost()]
        public HttpResponseMessage StateList(string request)
        {
            ...
            try
            {
                ...
                if ((apiKey!=null || apiKey!="") && requestIP!="")
                {
                    ...
                    AgentStateRequest objRQ = new AgentStateRequest();
                    AgentStateListReponse obj = new AgentStateListReponse();
                    if (objProcessor.IsJson)
                    {
                        objRQ = JsonConvert.DeserializeObject<AgentStateRequest>(requestString);
                        Log.Error.LogLogin(Convert.ToString(objRQ.CompanyID), Convert.ToString(requestString), "BBPSRequest");
                    }
                    else
                    {
                    ...
                    if (validate)
                    {
                        obj = objProcessor.StateListForBBPS(objRQ);
                    }
                    else
                    {
                    ...
                    if (objProcessor.IsJson)
                    {
                        ...
                        Log.Error.LogLogin(Convert.ToString(objRQ.CompanyID), Convert.ToString(json), "BBPSResponse");
